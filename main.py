...
def get_relative_transform(parent_pos, parent_quat, child_pos, child_quat):
    """
    Calculate the relative position and quaternion of a child body relative to its parent.
    
    Args:
    - parent_pos: Position of the parent body in world coordinates as a numpy array.
    - parent_quat: Quaternion of the parent body in world coordinates as a numpy array (w, x, y, z).
    - child_pos: Position of the child body in world coordinates as a numpy array.
    - child_quat: Quaternion of the child body in world coordinates as a numpy array (w, x, y, z).
    
    Returns:
    - relative_pos: Position of the child relative to the parent as a numpy array.
    - relative_quat: Quaternion of the child relative to the parent as a numpy array (w, x, y, z).
    """
    # Step 1: Calculate relative position in the parent's frame
    # Translate child position by the inverse of the parent's rotation
    parent_quat_conj = quaternion_conjugate(parent_quat)
    relative_pos = apply_quaternion(parent_quat_conj, child_pos - parent_pos)

    # Step 2: Calculate relative orientation (quaternion)
    relative_quat = quaternion_mult(parent_quat_conj, child_quat)
...
def add_component(env: Mujoco_XML, component_name: str, orig_data: Fusion_Model, remapped_data: Fusion_Model, stl_lookup_table: Dict[str, str], exclude_parent_list: List[str], quattrans_dict: Dict[str, np.ndarray]):
    # Finds the earliest parent of the component that is not in the exclude_parent_list
    def find_joint_parent(component: Fusion_Model.Component, exclude_parent_list: List[str]) -> Fusion_Model.Component:
        parent = component.parent
        while parent.name in exclude_parent_list:
    ...
    if component_name not in exclude_parent_list:
        ...
        if remapped_parent_name != "Root":
            print("Position before:", trans)
            print("Quaternion before:", quat)
            print("Parent position:", quattrans_dict[remapped_parent_name][1])
            print("Parent quaternion:", quattrans_dict[remapped_parent_name][0])
