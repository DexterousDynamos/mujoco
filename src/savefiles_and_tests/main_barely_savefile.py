...
# TODO: Tie M_APs to carpals, not root
...
def add_component(env: Mujoco_XML, component_name: str, orig_data: Fusion_Model, remapped_data: Fusion_Model, stl_lookup_table: Dict[str, str], exclude_parent_list: List[str]):
    ...
    # Calculates the total quaternion of the component
    def recursive_calculate_quat(component: Fusion_Model.Component, parent_name: str) -> np.ndarray:
        # if "M_PP" in component.name:
        #     print("Parent name for ", component.name, " is ", component.parent.name)
        # if component.name != "Root":
        #     print("Component name: ", component.name)
        #     print("Parent name: ", component.parent.name)
        if "M_PP" in component.name or "M_MP" in component.name:
            print("Calling recursive_calculate_quat for ", component.name)
        if component.name == parent_name:
            return component.quaternion
        return quaternion_mult(recursive_calculate_quat(component.parent, parent_name), component.quaternion)
    
    # Calculates the total translation of the component
    def recursive_calculate_trans(component: Fusion_Model.Component, component_translation: np.ndarray, parent_name: str) -> np.ndarray:
        # print("Component name: ", component.name)
        if component.name == parent_name:
            return component_translation
        return recursive_calculate_trans(component.parent, apply_quaternion(component.parent.quaternion, component_translation) + component.parent.translation, parent_name)
    ...
    if component_name not in exclude_parent_list:
        parent = find_joint_parent(remapped_component, exclude_parent_list)
        ...
        if parent_name == "Root":
            ...
            pass
            # print(f"Parent {parent_name} equivalent to {stl_lookup_table[parent_name]}")
        if "M_MP" in component_name:
            print("Parent name for ", component_name, " is ", parent_name)
        # print(orig_data)
        # print(orig_component.parent.name)
        quat = recursive_calculate_quat(remapped_component, parent_name)
        # if "Carpals" not in component.name:
        #     quat = quaternion_mult(quat, np.array([1, 1, 0, 0])*np.sqrt(2)/2) # [w, x, y, z], Fusion360 to Mujoco
        # print(f"Quaternion for {component.name}:", np.around(quat, 3))
        # trans = apply_quaternion(orig_component.parent.quaternion, orig_component.translation) + orig_component.parent.translation
        trans = recursive_calculate_trans(remapped_component, remapped_component.translation, parent_name)
        # print(f"Translation for {component.name}: ", np.around(trans, 3))
        # env.add_body(stl_lookup_table[component.name], pos=trans, quat=quat, parent=parent_name)
        # print("Parent name for ", component_name, " is ", parent_name)
...
if __name__ == "__main__":
    ...
    env.export_xml("xml/wrong_but_inherited.xml")
